...
class Packer<T>
{
	...
	var free : Array<Rectangle>;
	var used : Array<Packed<T>>;
	
	public function new(width:Int, height:Int, padding:Int, method:PackingMethod, rotate:RotateMethod)
	{
		this.width   = width;
		this.height  = height;
		...
		used = [];
		free = [ new Rectangle(0, 0, width, height) ];
	}
	...
	private function findTopLeft(width:Int, height:Int) : Rectangle
	{
		...
		for (f in free)
		{
			if (width > f.width || height > f.height) continue;
	...
	private function findBestShortSide(width:Int, height:Int) : Rectangle
	{
		...
		for (f in free)
		{
			...
			var cs = Std.min(f.width - width, f.height - height);
			if (ds < cs) continue;
			if (ds > cs)
			{
				ds = cs;
	...
	private function findBestLongSide(width:Int, height:Int) : Rectangle
	{
		...
		for (f in free)
		{
			...
			var cs = Std.max(f.width - width, f.height - height);
			if (ds < cs) continue;
			if (ds > cs)
			{
				ds = cs;
